...
export function Refinance({plan,onChange,snapshot,onOpenScenario}:{plan:PlannerData;onChange:(p:PlannerData)=>void;snapshot:RateSnapshot|null;onOpenScenario:(id:string)=>void}) {
 ...
 return <section className="planner-panel budget-flow refinance-panel"><h2>Refinance Comparison</h2>
  ...
  {plan.debts.map(d=>{
   ...
   return <details className="refinance-loan" key={d.id}><summary>{d.name||'Loan'} {opportunities.includes(d.id)&&<span className="opportunity-badge">Worth Comparing</span>}<small>{result?`${money(result.monthlyDifference)} monthly payment reduction · ${money(result.saving)} net saving over ${c.horizon} months`:missing.length?`${missing.length} offer fields needed`:'Review loan inputs'}</small></summary>
    ...
    <div className="budget-fields"><Amount unit="years" label={`New Term / Years — ${d.name}`} value={c.years} onChange={years=>write({years})}/><Amount percent label={`Offered Rate / % — ${d.name}`} value={c.rate} onChange={rate=>write({rate})}/><Amount label={`Closing Costs and Points — ${d.name}`} value={c.fees} onChange={fees=>write({fees})}/><Amount unit="months" label={`Holding Period / Months — ${d.name}`} value={c.horizon} onChange={horizon=>write({horizon})}/></div>
    ...
    {result?<div className="budget-flow refinance-result" aria-label={`Refinance Results — ${d.name}`}>
     ...
     <div className="budget-card budget-flow"><strong>Try This in Your Plan</strong><p>Save an opening-January {plan.budget.timeline?.startYear??'balance-year'} refinance scenario. Financed fees increase debt; upfront fees enter that month’s spending and funding calculation. Existing escrow amounts and extra-payment settings remain in place. The preview’s holding period does not end the household projection.</p><p className="field-help">Fixed-rate, no cash-out only. Rental loans, changed escrow/PMI, prepayment penalties, special loan tax treatment and mid-plan refinancing require separate modeling.</p><Button onClick={()=>save(c)}>Save Refinance Scenario</Button></div>
    </div>:<p role="status">{blockedStatement?'Reconcile the enabled mortgage statement to identify the P&I payment.':missing.length?`Still needed: ${missing.join(', ')}.`:'Use a positive term in whole months, a 1–600 whole-month holding period, and a positive balance with a payment that covers interest.'}</p>}
    {status&&<div className="budget-card" role="status">{status.messages.map(m=><p key={m}>{m}</p>)}{status.savedId&&<Button variant="outline" onClick={()=>onOpenScenario(status.savedId!)}>View Saved Refinance Scenario</Button>}</div>}
