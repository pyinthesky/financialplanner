...
export function PrintPlanDetails({plan}:{plan:PlannerData}){
  ...
  return <div className="report-plan-details">
    ...
    <section className="report-detail-section"><h2>Cash & Investments</h2><ReportTable headers={['Account','Owner','Tax Treatment','Opening Balance','Annual Contribution']} rows={plan.accounts.map(v=>[v.name,name(v.owner),name(v.kind),money(v.balance),money(v.annualContribution)])}/><p className="report-caption">Monthly planning uses linked payroll and budget transfers for new contributions. Account annual-contribution inputs belong to the annual engine.</p>
