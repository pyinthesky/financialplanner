...
export function Welcome({hasPlan,locked,onStart,onSample,onOpen,onNew}:{hasPlan:boolean;locked:boolean;onStart:()=>void;onSample:()=>void;onOpen:()=>void;onNew:()=>void}) {
  const steps=[
    {icon:PiggyBank,title:'Build Your Starting Point',copy:'Bring together your household, income, savings and debts.',color:'bg-blue-50 text-blue-700'},
    {icon:Sprout,title:'Shape Your Retirement',copy:'Decide what stays, what changes and what you want to make room for.',color:'bg-emerald-50 text-emerald-700'},
    {icon:Compass,title:'Explore Your Outlook',copy:'See how it adds up, then try different paths for your future.',color:'bg-violet-50 text-violet-700'},
  ];
  return <Stack className="welcome-page">
    <PlannerCard className="overflow-hidden bg-linear-to-br from-white via-white to-emerald-50/80">
      <div className="max-w-3xl py-3 md:py-7">
        <p className="mb-4 text-xs font-semibold tracking-[0.18em] text-emerald-700">YOUR LIFE. YOUR POSSIBILITIES.</p>
