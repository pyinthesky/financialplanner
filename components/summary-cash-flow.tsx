"use client";
import { ActionRow, PlannerCard } from '@/components/ui/planner-layout';
...
export function SummaryCashFlow({plan}:{plan:PlannerData}){
  ...
  return <PlannerCard as="section" stack ref={container} className="panel budget-flow summary-sankey"><div className="budget-toolbar"><h2>Cash Flow at a Glance</h2><ActionRow as="div" className="budget-actions" role="group" aria-label="Cash Flow Stage"><Button variant={retired?'outline':'default'} aria-pressed={!retired} onClick={()=>{setRetired(false);setSelected('');}}>Current</Button><Button variant={retired?'default':'outline'} aria-pressed={retired} onClick={()=>{setRetired(true);setSelected('');}}>In Retirement</Button></ActionRow></div>
