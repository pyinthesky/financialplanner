...
export function SummaryCashFlow({plan}:{plan:PlannerData}){
  ...
  const width=mobile?360:1000,height=Math.max(440,Math.max(flow?.sources.length??0,flow?.uses.length??0)*58),top=45,gap=26,available=height-top-30-gap*Math.max(flow?.sources.length??0,flow?.uses.length??0);
  const scale=flow?.total?available/flow.total:0,center=width/2;
  const left=mobile?5:205,right=mobile?width-11:width-205;
  const side=(parts:FundingPart[],incoming:boolean)=>{let outer=top,inner=top;return parts.map((p,i)=>{const h=p.amount*scale,y=outer,cy=inner;outer+=h+gap;inner+=h;const x=incoming?left+6:center+4,end=incoming?center-4:right;const sy=incoming?y:cy,ey=incoming?cy:y;const tx=incoming?(mobile?14:10):(mobile?width-14:right+14);const name=p.name.length>(mobile?22:27)?p.name.slice(0,mobile?21:26)+'…':p.name;return <g key={p.name}><path d={`M ${x} ${sy} C ${center} ${sy},${center} ${ey},${end} ${ey} L ${end} ${ey+h} C ${center} ${ey+h},${center} ${sy+h},${x} ${sy+h} Z`} fill={colors[i%colors.length]} opacity="0.3"><title>{p.name}: {money(p.amount)}</title></path><rect x={incoming?left:right} y={y} width="6" height={h} fill={colors[i%colors.length]}/>{mobile&&<rect x={incoming?12:width-166} y={y-3} width="154" height="32" fill="white" opacity="0.92" rx="3"/>}<text x={tx} y={y+9} fontSize={mobile?10:12} textAnchor={!incoming&&mobile?'end':'start'} fill="#334155">{name}<title>{p.name}</title></text><text x={tx} y={y+23} fontSize={mobile?11:13} fontWeight="600" textAnchor={!incoming&&mobile?'end':'start'} fill="#152237">{money(p.amount)}</text></g>;});};
  return <section className="panel budget-flow summary-sankey"><div className="budget-toolbar"><h2>Cash Flow at a Glance</h2><div className="budget-actions" role="group" aria-label="Cash Flow Stage"><Button variant={retired?'outline':'default'} aria-pressed={!retired} onClick={()=>{setRetired(false);setSelected('');}}>Current</Button><Button variant={retired?'default':'outline'} aria-pressed={retired} onClick={()=>{setRetired(true);setSelected('');}}>In Retirement</Button></div></div>
    ...
    {!result.supported?<div role="status"><p>Complete the monthly plan to see its cash-flow diagram.</p><ul>{result.issues.map(s=><li key={s}>{s}</li>)}</ul></div>:!row?<p role="status">This retirement date is outside the planning horizon. Adjust it in Household.</p>:flow&&<><label className="budget-field"><span>Sankey Month</span><select aria-label="Sankey Month" value={month} onChange={e=>setSelected(e.target.value)}>{result.months.map(r=><option key={r.month}>{r.month}</option>)}</select></label>
      {flow.unfunded>0&&<p role="status">Unfunded obligations: {money(flow.unfunded)}. Only paid amounts appear in the flow.</p>}
      {Math.abs(flow.residual)>.01?<p role="alert">The cash flow does not reconcile; review the monthly ledger.</p>:flow.total===0?<p>No funded flows in this month.</p>:<svg className="summary-sankey-svg" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${retired?'Retirement':'Current'} Cash Flow Sankey for ${month}; funding and uses each ${money(flow.total)}`}><text x={mobile?10:205} y="20" fontSize="14">Funding</text><text x={mobile?width-10:right} y="20" textAnchor={mobile?'end':'start'} fontSize="14">Uses</text>{side(flow.sources,true)}<rect x={center-4} y={top} width="8" height={flow.total*scale} fill="#334155"/>{side(flow.uses,false)}</svg>}
