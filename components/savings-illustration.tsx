...
export function SavingsIllustration() {
  ...
  const x = (rate:number) => 58+(rate-5)/90*562;
  const y = (horizon:number) => 260-horizon/ceiling*224;
  ...
  return <PlannerCard stack data-savings-illustration>
    ...
    {!active ? <div><Button onClick={()=>setActive(true)}>Explore the Savings Curve</Button><p className="mt-2 text-xs text-muted-foreground">Loads a separate illustration with 25% saving, 5% real return and a 4% withdrawal assumption. Your plan stays unchanged.</p></div> : <>
      ...
      <svg viewBox="0 0 640 310" role="img" aria-labelledby={`${id}-title ${id}-desc`} className="block h-auto w-full min-w-0 rounded-lg bg-secondary/40">
        ...
        {[0,1,2,3,4].map(i=>{const n=ceiling*i/4;return <g key={i}><line x1={58} x2={620} y1={y(n)} y2={y(n)} stroke="var(--border)"/><text x={49} y={y(n)+4} textAnchor="end" fontSize={12} fill="var(--muted-foreground)">{Math.round(n)}</text></g>;})}
        <text x={58} y={19} fontSize={12} fill="var(--muted-foreground)">Years to Target</text>
        <path d={`${path} L620,260 L58,260 Z`} fill="var(--primary)" opacity={0.08}/>
        <path d={path} fill="none" stroke="var(--primary)" strokeWidth={3}/>
        <line x1={x(saving)} x2={x(saving)} y1={y(years)} y2={260} stroke="var(--primary)" strokeDasharray="4 4"/>
        <circle cx={x(saving)} cy={y(years)} r={6} fill="var(--primary)" stroke="white" strokeWidth={2}/>
        {[5,25,50,75,95].map(rate=><text key={rate} x={x(rate)} y={281} textAnchor="middle" fontSize={12} fill="var(--muted-foreground)">{rate}%</text>)}
        <text x={339} y={303} textAnchor="middle" fontSize={12} fill="var(--muted-foreground)">Savings Rate</text>
      </svg>
