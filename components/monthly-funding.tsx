...
export function MonthlyFunding({rows}:{rows:MonthlyRow[]}){
 ...
 <div className="funding-columns">{[{title:'Funding',parts:flow.sources},{title:'Uses',parts:flow.uses}].map(group=><div key={group.title}><h4>{group.title} · {money(flow.total)}</h4>{group.parts.map((p,i)=><div className="funding-part" key={p.name}><div><span>{p.name}</span><strong>{money(p.amount)}</strong></div><div className="funding-track"><span style={{width:`${flow.total?p.amount/flow.total*100:0}%`,background:colors[i%colors.length]}}/></div></div>)}</div>)}</div><p className="field-help">Cash at start {money(row.openingCash)} → cash at end {money(row.cash)}. Reconciliation difference: {flow.residual.toFixed(6)}.</p></PlannerCard>;
}
