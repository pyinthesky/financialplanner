...
export function MortgageStatementEditor({ statement, debts, onChange }: { statement?: MortgageStatement; debts: Debt[]; onChange: (s: MortgageStatement) => void }) {
  ...
  return <section className="planner-panel budget-flow"><h2>Reconcile a Mortgage Statement</h2><p>Your lender's payment may include taxes and insurance. Only principal and interest can pay down the loan or roll into the next debt.</p>
    ...
    <div className="budget-fields">{([['total','Total Statement Payment'],['principalInterest','Principal & Interest'],['propertyTax','Property Tax Escrow'],['insurance','Home Insurance Escrow'],['mortgageInsurance','Mortgage Insurance'],['otherEscrow','Other Escrow']] as const).map(([key,label]) => <label className="budget-field" key={key}><span>{label} / Month</span><div className="input-affix"><span>$</span><Input aria-label={label} type="number" min={0} step="0.01" value={s[key] ?? ''} onFocus={e => e.currentTarget.select()} onChange={e => onChange({ ...s, [key]: e.target.value === '' ? null : e.target.valueAsNumber })} /></div></label>)}</div>
