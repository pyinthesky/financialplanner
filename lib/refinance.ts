...
export function refinanceOpportunities(plan:PlannerData,snapshot:unknown,now=new Date()){
 ...
 return (plan.refinancing??[]).filter(c=>{const d=plan.debts.find(d=>d.id===c.debtId),rate=snapshot.rates.find(r=>r.years===c.years);return d?.kind==='mortgage'&&!plan.realEstate?.properties.some(p=>p.debtId===d.id)&&d.balance>0&&c.benchmarkEligible&&rate&&d.interestRate>rate.rate&&(!c.snoozeUntil||c.snoozeUntil<now.toISOString().slice(0,10));}).map(c=>c.debtId);
}
