> September 11 benefit-mapping update: [the current mapping and yearly refresh contract](fehb-refresh.md) supersedes the price-only copying and manual family-limit workflow below. Nine detailed nationwide profiles and narrow catalog-wide mappings now populate supported rules; unmapped cases remain explicit. No certification checkboxes are required.

> September 11, 2026 usability update: the current implementation supersedes earlier certification requirements in this design. Entered benefits, HRA allowances, surcharges and waiver amounts are sufficient to calculate an estimate. Factual missing inputs are highlighted in the form and their sections open automatically. No benefit-review checkbox or care fingerprint blocks ranking. Effective-year, catalog-version, service-area, covered-group and numerical consistency checks remain.
...
- Compact directory, geography filters, 12 separate federal review slots, automatic source-backed service rules, source text and links, remaining family/formulary inputs, top-three full-household shortlist, pin/show-all controls and reference-version validation and automatic care recalculation. Only January 2026 comparisons use this catalog. Government contributions are not counted as employee cash benefits.
- Per-employer gross/net waivers and group-dependent eligibility, proration and payment timing; surcharges; two-group comparisons with independent accumulators and full coverage checks. Waiver income disables the automatic Household-only HSA tax estimate because taxable gross can vary by coverage choice; users supply a reviewed rate. Shared HSA allowances require an entered limit; automatic split-plan contribution allocation and dual coverage are excluded.
...
## Premium Tax Savings and Entry Guidance

[OPM premium conversion](https://www.opm.gov/healthcare-insurance/healthcare/reference-materials/premium-conversion/) has allowed eligible federal employees to pay FEHB premiums before tax since October 2000 (source checked September 11, 2026). Savings may include federal income, FICA and state/local taxes. The tool explains this optional benefit; it does not fabricate a combined tax percentage from the federal bracket. Blank premium tax rate means the full premium is compared before tax savings. An entered zero represents no premium tax benefit. The optional HSA rate follows the existing Household-derived estimate when supported; otherwise zero tax savings are displayed without blocking annual cost or cash-flow results.

The HRA model uses the entered allowance available up front and reimburses actual covered medical/Rx payments, capped at that allowance. No checkbox attestation is required. An HRA of zero does not conflict with HSA calculations; a positive combined HSA/HRA arrangement remains outside the supported model. Waiver amounts represent payments the user expects to receive when accepting the other employer's coverage. Surcharges are amounts additional to entered premiums. Existing saved confirmation booleans remain readable for compatibility but no longer gate costs or ranking.
