...
### September 7 enhanced-budget / monthly-laboratory build

User explicitly authorized enhanced current/retirement budgets and the rest of Batch 5. Timed builds stay off. Generic layout requirements are in batch-5-experience.md; no private reference assets or financial content are in the working changes.

Implemented locally: budget presentation grouping, Today/Retirement values, savings destinations, payroll reconciliation, retirement months, dated edits, and planned monthly budget; lib/monthly-projection.ts runs month-end returns with January opening balances and December annual tax settlement using existing sourced modules. Payroll savings/employer contributions are distinct; legacy automatic account contributions are not credited in this engine. Monthly identities, scheduled payments, current/retirement transitions, tax-funding, missing input gates, scenario equality/staleness and sequence effects have independent synthetic tests. Scenario Laboratory offers immutable local snapshots, explicit rebase, comparison of up to three, selected overrides, coverage targets/refill, discretionary floor guardrails, cash events, and a legacy target. A switch selects the monthly results for Plan Summary/PDF; legacy annual worksheets remain separately labeled.

Do not confuse implementation with release verification: run the expanded Chromium/WebKit journey, inspect the populated monthly PDF, and record the actual Pages outcome before claiming deployment.

Scope limits needing follow-up: equal-budget mortgage payoff/investing, home sale/downsize and exclusion rules, broad reference catalogs/ZIP, automatic claiming/survivor/state rules, actual transaction tracking and reserve-account earmarking. Monthly benefits still use entered annual amounts and age-based January starts. HSA reimbursements remain reserved/unmodeled. Separate tax returns are gated. Roth access is conservative: confirmed eligible pretax conversion sources, owner-matched destination, five-tax-year locks on converted lots, confirmed qualified Roth year and age 60. IRA basis, loss-lot accounting, quarterly estimated-tax compliance, premiums driven by ACA/IRMAA and specialized RMD rules are not silently inferred. Cash refill skips December after settlement; unpaid debt obligations explicitly invalidate achieved debt-balance assumptions. Initial simple view groups the same bills; it does not yet implement aggregate-envelope/detail allocation.

Authoritative source review September 7, 2026: IRS Publication 590-B (2025), https://www.irs.gov/publications/p590b; Publication 505 (2026), https://www.irs.gov/publications/p505; Topic 558, https://www.irs.gov/taxtopics/tc558. These establish the limits around distributions, conversions and pay-as-you-go tax; the December settlement is explicitly a planning convention, not payment-compliance advice. Existing 2026 tax constants are reused unchanged.

Read [the experience contract](batch-5-experience.md) alongside the calculation contract. Carry forward compact grouped budgets, independent flexibility/priority/timing dimensions, aggregate-versus-detail reconciliation, scoped edits, and planned/actual distinctions into the monthly schema. The document contains generic requirements only; private visual references and personal financial content must never enter repository assets, fixtures, logs, or uploaded artifacts.
