...
module.exports=async function checkEnrollment({page,navigate,noOverflow,name,width,out}){
  ...
  const limits=review.getByText('Deductibles & Limits',{exact:true});
  if(!await review.getByLabel('Per-Person OOP Max',{exact:true}).isVisible())await limits.click();
  assert.equal(await review.getByLabel('Per-Person OOP Max',{exact:true}).inputValue(),'6000','Brochure supplies the embedded family OOP cap');
  assert.equal(await review.getByLabel('Family Deductible Structure',{exact:true}).inputValue(),'aggregate');
  assert.equal(await review.getByLabel('Family Deductible',{exact:true}).inputValue(),'3600');
  assert.equal(await page.locator('main .oe-result').count(),3,'Incomplete federal care does not displace private comparisons');
  await review.getByLabel('Formulary Tier & Fill',{exact:true}).selectOption('rx2');
  await review.getByLabel('Published Service',{exact:true}).first().selectOption('primary');
  await review.getByLabel('Personal Contribution / Year',{exact:true}).fill('0');
  await review.getByLabel('Opening HSA Balance',{exact:true}).fill('0');
  await review.getByRole('button',{name:'Use 2026 Full-Year Base Limit',exact:true}).click();
  assert.equal(await page.locator('main .oe-result').count(),4,'Source-backed federal rules produce a comparison without re-entering benefits');
